...
function bind(naisa) {
    ...
    naisa.ev.on('connection.update', update => {
        Object.assign(state, update)
    })

    naisa.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    naisa.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    naisa.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
   ...
   naisa.ev.on('contacts.update', updates => {
        for (const update of updates) {
            Object.assign(contacts[update.id], update)
        }
    })

    naisa.ev.on('chats.upsert', newChats => {
        chats.upsert(...newChats)
    })

    naisa.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    naisa.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    naisa.ev.on('chats.update', updates => {
    ...
    naisa.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    naisa.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
    ...
    /**naisa.ev.on('messages.update', updates => {
    ...
    naisa.ev.on('groups.update', updates => {
        for (const update of updates) {
    ...
    naisa.ev.on('group-participants.update', ({ id, participants, action }) => {
    ...
    naisa.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (naisa === null || naisa === void 0 ? void 0 : naisa.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id) => {
            ...
            if (!message) {
                message = await (naisa === null || naisa === void 0 ? void 0 : naisa.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (naisa === null || naisa === void 0 ? void 0 : naisa.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return naisa === null || naisa === void 0 ? void 0 : naisa.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (naisa === null || naisa === void 0 ? void 0 : naisa.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (naisa === null || naisa === void 0 ? void 0 : naisa.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (naisa === null || naisa === void 0 ? void 0 : naisa.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (naisa === null || naisa === void 0 ? void 0 : naisa.messageInfo(remoteJid, id))
                if (msg) {
